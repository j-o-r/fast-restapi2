 ...
 * ClientWrapper
 *
 * Wrapper arround the response en request objects
 */
class ClientWrapper {
  /**
   * @param {Object} req - Class: http.IncomingMessage
  ...
  constructor (req, res, query) {
    ...
    this.query = query;
    /** {object} headerFields - headers to output */
    this.headerFields = {};
    /** {string} stream_node namespace to start streaming e.g. {namespace:[ */
    this.stream_node = '';
   ...
   * Get posted data
   *
   * @param  {number} [maxSize=1024] - max body size
  ...
  getPost (maxSize) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let contentLength = self.req.headers['content-length'];
      if (contentLength && parseInt(contentLength) > maxSize) {
        reject(new Error('Post Body max size exceeded'));
        return;
      }
      // We only accept JSON
   ...
   *
   * @param  {string} [ns] - Start of an array like json output
   * @returns {void}
   */
  openStream (ns) {
