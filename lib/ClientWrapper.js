...
/**
 * Serve a folder, but skip fikles starting with a .
 *
      ...
      files.forEach((entry) => {
        ...
        if (!/^\./.test(entry.name)) {
          if (entry.isDirectory()) {
            ...
            client.stream({name: entry.name, type: 'file'});
          }
        }
      });
      client.closeStream();
  ...
  form.parse(req,
    (err, fields, postFiles) => {
      ...
      for (let file in postFiles) {
        let fname = postFiles[file].originalFilename;
        if (!isFileName(fname)) {
...
class ClientWrapper {
  ...
  getPost () {
    ...
    return new Promise((resolve, reject) => {
      const method = self.req.method === 'POST' || self.req.method === 'PUT' || self.req.method === 'PATCH' || false;
      if (!method) {
