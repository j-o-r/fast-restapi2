...
const requestHandler = async (req, res) => {
	...
	if (Object.keys(action).length > 0) {
		...
		// async || sync
		try {
			if (jsType(api[action.action]) === 'Function') {
			  api[action.action](client, action.params);
			} else {
			  await api[action.action](client, action.params);
			}
		} catch (_e) {
			const message = new Error('Unhandled Api Error');
			client.serve(500, message.toString());
		}
	} else {
